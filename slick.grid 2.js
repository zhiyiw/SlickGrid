...
(function($) {
    ...
    function SlickGrid(container,data,columns,options) {
        ...
        function updateIndexAfter(row, increase){
            var keys = [];
            var key;
            var int_index;
            increase = increase? 1:-1;
            for(var index in customRowHeights){
                int_index = parseInt(index)
                if (int_index <= row) continue
                keys.push(int_index)
            }
            for(var i = 0; i < keys.length; i++){
                key = keys[i]
                customRowHeights[key + increase] = customRowHeights[key]
                delete customRowHeights[key]
            }
        }
        function setRowHeight(row, height) {
            var need_to_update = false;

            if( (typeof customRowHeights[row] != "undefined" && height == null)  // this is when deleting an existing row
                || (typeof customRowHeights[row] == "undefined" && height != null) ){ // this is when adding a new row
                need_to_update = true;
            }

            if (height == null) {
              delete customRowHeights[row];
              if (need_to_update) updateIndexAfter(row, false)

            } else {
              customRowHeights[row] = height;
              if (need_to_update) updateIndexAfter(row, true)
            }
        }
        function getRowHeights(){
            return customRowHeights;
        }
        function setRowHeights(row_heights){

            customRowHeights = row_heights;
            updateIndexAfter(0, false)
        }
        ...
        function updateRowCount() {
            var newRowCount = getDataLength() + (options.enableAddRow?1:0) + (options.leaveSpaceForNewRows?1:0);//NOTE: this is changed
            var oldH = h;
